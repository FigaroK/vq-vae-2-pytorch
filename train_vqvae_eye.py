...
from dataset import _all_image_paths, _split_dataset, list_dataset, H5_EYE_dataset, identity
...
def train(epoch, train_loader, valid_loader, model, optimizer, scheduler, device, best_stats_dict, vis=None):
    ...
    for i, (img, left_label, left_headpose) in enumerate(loader):
        model.zero_grad()
        model.train()
        ...
        if iteration % 2000 == 0 and valid_loader:
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(f"/disks/disk2/fjl/checkpoint/vq-vae_{args.beta}_all/")
    os.makedirs(model_save_path, exist_ok=True)
    train_paths = glob.glob(f"{path}/all.h5")
    valid_paths = glob.glob(f"{path}/P[0-9]*_valid.h5")
    ...
    model = nn.DataParallel(VQVAE(in_channel=3)).to(device)
    ...
    for i in range(config.n_epoch):
        if config.sched == 'plateau':
            ...
            loss = train(i, train_loader, None, model, optimizer, scheduler, device, best_stats_dict, vis)
